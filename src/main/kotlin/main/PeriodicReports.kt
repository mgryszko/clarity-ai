...
class PeriodicReports : CliktCommand() {
    ...
    private val reportFreqMs: Long by option("-f", "--frequency", help = "reporting frequency [ms]").long().default(oneHour)
    private val maxTolerableLagMs: Long by option("-l", "--max-lag", help = "maximum tolerable lag of log entries [ms]")
        .long().default(fiveMinutes)
    private val timeoutMs: Long by option("-t", "--timeout", help = "log inactivity timeout [ms]").long().default(thirtySeconds)
